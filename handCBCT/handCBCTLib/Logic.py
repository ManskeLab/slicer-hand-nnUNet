...
<<<<<<< HEAD
		...
		def __init__(self, logMethod, errorMethod, finishSignal = None):
			...
			:param finishSignal: provide a callback as signal for segmentation finish, None by default
			"""
...
=======
    ...
    def cleanSegmentation(self, segmentationNode, progressBar = None):
      """
      Takes in a segmentation node and processes by removing all but largest island for each segment

      :param segmentationNode: the segmentation node to process
      :param progressBar: optional progress bar for dynamic progress updates
      """
      ...
      if progressBar:
        progressBar.setLabelText("Processing segmentation. This may take some time.")
      else:
        self.logMethod("Processing segmentation. This may take some time.")
      ...
      try:
        ...
        if progressBar:
          progressBar.maximum = segmentation.GetNumberOfSegments()

        for segmentID in segmentIDs:
          
          # update progress bar for each segment completed
          if progressBar:
            progressBar.value += 1
            slicer.app.processEvents()
